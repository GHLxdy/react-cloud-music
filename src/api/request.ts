...
export const loginByPhoneRequest = <T=any>(phone:string, password:string) => {
  return axiosInstance.get<T>(
    `/login/cellphone?phone=${phone}&password=${password}`
  );
};

export const sentVcodeRequest = <T=any>(phone:string) => {
  return axiosInstance.get<T>(`/captcha/sent?phone=${phone}`);
};

export const loginByVcodeRequest = <T=any>(phone:string, vcode:string) => {
  return axiosInstance.get<T>(`/captcha/verify?phone=${phone}&captcha=${vcode}`);
};